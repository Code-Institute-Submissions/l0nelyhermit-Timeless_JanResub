...
# Instance Variable for the Maximum Number Of Users,
# Posts and Listings for the site
...
def register_user():
    if request.method == "GET":
        return render_template('register_user.template.html',
                               questions=all_security_questions,
                               errors={}, countries=country_list)
    else:
        ...
        if (fname or dob  or username or password or securityanswer) == "" or None:
            ...
            if len(errors) > 0:
                    ...
                    return render_template('register_user.template.html',
                                           questions=all_security_questions,
                                           errors=errors,countries=country_list)
        # Check if Full Name entered is valid
        elif len(fname) < 3 or len(fname) > 70:
        ...
        # Check if the User already exists in the database
            ...
            # SCENARIOS:
            # 1. If user enters same username as
            # another exisitng user, raise error
            # 2. If user enters same email as
            # another existing user, raise error
            # 3. If user enters same username
            # and same email as another existing user, raise error
            # 4. If user does not exist, proceed with creation

            # if username does not match with any existing in database
            if not username_check:
                if email_check:
                    # If user enters same email as another
                    # existing user, raise error
                    if email == email_check['Email']:
                        flash("""Error: Email has already been used,
                              please try again with another
                            valid email','danger""")
                        errors.update(invalid_email="""Email has already
                                      been used, please enter a valid email""")
                        database_error = True

                        if len(errors) > 0:
                            flash("Registration Failure", 'danger')
                            return render_template(
                                            'register_user.template.html',
                                            questions=all_security_questions,
                                            errors=errors,countries=country_list
                                            )
                else:
                    if len(errors) > 0:
                        ...
                        # that is unique and not repeated
                        account_number_list = list(range(1,
                                                         max_capacity_users))
                        random.shuffle(account_number_list)
                        number = account_number_list.pop()
                        # Issues a random ID number for the profile
                        # ID that is unique and not repeated
                        profile_number_list = list(range(1,
                                                         max_capacity_users))
                        ...
                        # Using Flags Technique, if account
                        ...
                        if create_account_success is True:
                ...
                if username_check['Username'] == username:
                    # If user enters username that matches another
                    # existing username, raise error
                    flash("""Error: account already exists,
                          please try again with
                          another valid username','danger""")
                    errors.update(invalid_username="""Account
                    ...
                    # if user enters username and email that
                    # matches another existing username and email, raise error
                    if email_check:
                        if (email_check['Email'] == email) and (username_check['Username'] == username):
                            flash("""Error: Email and Username
                                  have been used already,
                                  please enter something different','danger""")
                            errors.update(invalid_email_and_username="""Email and
                                          Username have been used,
...
def login_user():
    ...
    # Login Validation
        ...
        if check_user and pbkdf2_sha256.verify(password,
                                               check_user['Password']):
            user_logged_in = User()
...
def show_user_profile():
    ...
    return render_template('show_profile.template.html',
...
def edit_user_profile():
    if request.method == 'GET':
        ...
        return render_template('update_profile.template.html',
                               current_profile=current_profile)
    else:
        ...
        if len(fname) < 3 or len(fname) > 70:
            ...
            # raise error and redirect to edit user profile form
            if len(errors) > 0:
...
# Creation of the Post System, Delete My Posts,
# Edit My Posts, See My Posts
...
def create_post():
    if request.method == "GET":
        ...
        if title == "":
            ...
            errors.update(invalid_title="""Title Field is Empty,
                          please enter a valid title""")
        # Check if content is empty
        elif content == "":
            flash("Error: Invalid Content", 'danger')
            errors.update(invalid_content="""Content Field is Empty,
        ...
        # If no errors, insert post data into database
            ...
            # Issues a random ID number for
            # the profile ID that is unique and not repeated
...
def show_user_posts():
    ...
    # Get the current page number from the args get request,
    # if it does not exist set it to zero
    page_number = request.args.get('page_number') or '0'
    page_number = int(page_number)
    # calculate how many results will be
    # skipped depending on page number
    ...
    return render_template('user_posts.template.html',
...
def show_post(post_id):
    if request.method == "GET":
        ...
        return render_template('show_post.template.html',
                               post=post,
                               page_number=page_number,
                               number_of_pages=number_of_pages)
    else:
...
def edit_user_posts(post_id):
    if request.method == 'GET':
        ...
        return render_template('edit_user_post.template.html',
                               user_post=user_post)
    else:
        ...
        # if the fields are empty raise the error
        # Accumulator
        errors = {}
        # Check if title is empty
        if title == "":
...
def edit_comment(comment_id):
    ...
    if request.method == "GET":
        ...
        if flask_login.current_user.username == user_post:
            return render_template('edit_comment.template.html',
        ...
        # Validate the field, check if the comment is not empty.
        ...
        if comment == "":
            ...
            db.Posts.update({
                ...
                'Comments.Content': str(comment_data.get
...
def create_listing():
    if request.method == 'GET':
        ...
        # If no errors, insert listing data into database
            ...
            # Issues a random ID number for the
            # profile ID that is unique and not repeated
...
def edit_listing(listing_id):
    ...
    get_id = listing['_id']
    if request.method == 'GET':
        return render_template('edit_listing.template.html',
                               listing=listing,
                               countries=country_list,
                               brands=brands_list,
                               cloud_name=CLOUD_NAME,
...
def show_user_listings():
    ...
    return render_template('user_listings.template.html',
...
def search_listings_marketplace():
    ...
    # Creating the query based on the search terms
        ...
        return render_template('marketplace.template.html',
...
def search():
    if request.method == 'GET':
        return render_template('global_search.template.html',
                               brands=brands_list_search)
    else:
        ...
        if user_choice_search == 'listing':
            ...
            # to skip depending on the page number
            number_to_skip = page_number * page_size

            all_listings = (db.Listings.find(criteria)
                            .skip(number_to_skip).limit(page_size))
            # Pass the data to the template
            return render_template('search_listings_results.template.html',
                                   listings=all_listings,
                                   page_number=page_number,
                                   number_of_pages=number_of_pages)
        else:
            ...
            return render_template('search_posts_results.template.html',
                                   posts=all_posts,
                                   page_number=page_number,
                                   number_of_pages=number_of_pages)
